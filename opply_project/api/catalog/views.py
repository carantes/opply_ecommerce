...
from api.catalog.serializers import ProductSerializer, InventoryReserveSerializer, InventoryShipSerializer
...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    # @action(detail=True, methods=['post'], serializer_class=InventoryReserveSerializer)
    # def reserve(self, request, pk=None):
    #     '''
    #     API endpoint that allows a product item to be reserved on the inventory.
    #     '''
    ...
    #         return Response({'status': 'Product was reserved'}, status=status.HTTP_200_OK)
